...
public sealed class SkillInstallationScanner
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        foreach (var skillDirectory in Directory.EnumerateDirectories(fullTargetRoot).Order(StringComparer.Ordinal))
        {
            ...
            var hostMatchResult = await hostInspector.MatchesHostAsync(resolvedSkillDirectory, manifest, host, cancellationToken).ConfigureAwait(false);
            ...
            installedSkills.Add(new SkillInstalledSkill(
                new SkillInstallIdentity(host, SkillScopeKind.Project, fullTargetRoot, manifest.SkillName),
                resolvedSkillDirectory,
                manifest));
        }
