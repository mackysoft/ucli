...
internal sealed class DaemonGuiStartupObserver : IDaemonGuiStartupObserver
{
    ...
    private async ValueTask<DaemonGuiStartupBlockerObservation?> TryClassifyLogAsync (
        ResolvedUnityProjectContext unityProject,
        int processId,
        DateTimeOffset processStartedAtUtc,
        string unityLogPath,
        CancellationToken cancellationToken)
    {
        ...
        return new DaemonGuiStartupBlockerObservation(
            ...
            processId,
            processStartedAtUtc,
            string.IsNullOrWhiteSpace(logReadResult.Path) ? unityLogPath : logReadResult.Path);
    }
