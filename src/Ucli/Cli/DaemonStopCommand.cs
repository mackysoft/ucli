...
{
    private readonly IDaemonStopCommandService daemonStopCommandService;

    /// <summary> Initializes a new instance of the <see cref="DaemonStopCommand" /> class. </summary>
    /// <param name="daemonStopCommandService"> The daemon-stop command service dependency. </param>
    /// <exception cref="ArgumentNullException"> Thrown when <paramref name="daemonStopCommandService" /> is <see langword="null" />. </exception>
    public DaemonStopCommand (IDaemonStopCommandService daemonStopCommandService)
    {
        this.daemonStopCommandService = daemonStopCommandService ?? throw new ArgumentNullException(nameof(daemonStopCommandService));
    }
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        var executionResult = await daemonStopCommandService.Stop(projectPath, timeout, cancellationToken).ConfigureAwait(false);
