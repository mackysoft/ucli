...
internal static class ProjectContextErrorCodes
{
    ...
    public static readonly UcliCodeValue ProjectPathInvalidFormat = new("PROJECT_PATH_INVALID_FORMAT");

    /// <summary> Gets the error code used when a project path does not exist. </summary>
    public static readonly UcliCodeValue ProjectPathNotFound = new("PROJECT_PATH_NOT_FOUND");

    /// <summary> Gets the error code used when a directory is missing required Unity project markers. </summary>
    public static readonly UcliCodeValue UnityProjectMarkerMissing = new("UNITY_PROJECT_MARKER_MISSING");

    /// <summary> Gets the error codes owned by project-context resolution. </summary>
    public static IReadOnlyCollection<UcliCodeValue> All { get; } =
    [
    ...
    public static bool Contains (UcliCodeValue? errorCode)
    {
