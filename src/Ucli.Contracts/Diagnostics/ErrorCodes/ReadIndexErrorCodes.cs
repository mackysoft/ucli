...
public static class ReadIndexErrorCodes
{
    ...
    public static readonly UcliCodeValue ReadIndexBootstrapFailed = new("READ_INDEX_BOOTSTRAP_FAILED");

    /// <summary> Gets the error code emitted when read-index files are malformed. </summary>
    public static readonly UcliCodeValue ReadIndexFormatInvalid = new("READ_INDEX_FORMAT_INVALID");

    /// <summary> Gets the error code emitted when a request requires fresh read-index but freshness is not <c>fresh</c>. </summary>
    public static readonly UcliCodeValue ReadIndexFreshRequired = new("READ_INDEX_FRESH_REQUIRED");
}
