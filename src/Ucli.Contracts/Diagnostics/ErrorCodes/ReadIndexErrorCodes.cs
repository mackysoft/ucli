...
public static class ReadIndexErrorCodes
{
    ...
    public static readonly UcliErrorCode ReadIndexBootstrapFailed = new("READ_INDEX_BOOTSTRAP_FAILED");

    /// <summary> Gets the error code emitted when read-index files are malformed. </summary>
    public static readonly UcliErrorCode ReadIndexFormatInvalid = new("READ_INDEX_FORMAT_INVALID");

    /// <summary> Gets the error code emitted when a request requires fresh read-index but freshness is not <c>fresh</c>. </summary>
    public static readonly UcliErrorCode ReadIndexFreshRequired = new("READ_INDEX_FRESH_REQUIRED");
}
