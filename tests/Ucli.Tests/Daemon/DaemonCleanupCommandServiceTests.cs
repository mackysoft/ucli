...
public sealed class DaemonCleanupCommandServiceTests
{
    [Fact]
    [Trait("Size", "Small")]
    public async Task Cleanup_WhenOperationReturnsCompleted_ReturnsCompletedOutput ()
    {
        var context = DaemonCommandServiceTestContext.CreateExecutionContext(timeoutMilliseconds: 3400);
        var resolver = new DaemonCommandServiceTestContext.StubDaemonCommandExecutionContextResolver(
            DaemonCommandExecutionContextResolutionResult.Success(context));
        var operation = new DaemonCommandServiceTestContext.StubDaemonCleanupOperation
        {
            CleanupResult = DaemonCleanupResult.Completed(),
        };
        var service = new DaemonCleanupCommandService(resolver, operation);
    ...
    {
        var context = DaemonCommandServiceTestContext.CreateExecutionContext(timeoutMilliseconds: 3100);
        var resolver = new DaemonCommandServiceTestContext.StubDaemonCommandExecutionContextResolver(
            DaemonCommandExecutionContextResolutionResult.Success(context));
        var operation = new DaemonCommandServiceTestContext.StubDaemonCleanupOperation
        {
