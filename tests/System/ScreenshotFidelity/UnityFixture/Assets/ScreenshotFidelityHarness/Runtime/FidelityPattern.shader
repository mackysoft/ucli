    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            float4 Pattern (float2 uv)
            {
                ...
                float2 cornerSeparator = max(float2(0.07, 0.07), 20.0 / _ScreenParams.xy);
                if ((uv.x < cornerSeparator.x || uv.x > 1.0 - cornerSeparator.x)
                    && (uv.y < cornerSeparator.y || uv.y > 1.0 - cornerSeparator.y))
