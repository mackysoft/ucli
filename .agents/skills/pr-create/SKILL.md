---
name: pr-create
description: PR作成依頼を受けたときに、テスト・`$push`・PR作成までを一気通貫で実行する。Issueがある場合はIssueを優先して本文とbaseを決定し、Issueが特定できない場合は確認なしでIssueなし運用として継続する。既存Open PRがある場合は更新か新規作成を確認する。
---

# 目的
- PR作成手順を固定し、テスト漏れ・push漏れ・本文品質のばらつきを防ぐ。
...
- Issueが特定できない場合は、確認なしでIssueなし運用として継続する。
...
1. Issue番号は次の優先順で解決する。
   - 明示入力
   - ブランチ名の `issue-<N>-...`
2. Issueが解決できた場合は、`gh issue view` でIssue本文を取得し、`Base branch` 指定を優先して base を確定する。
3. Issueが解決できない場合は、確認せず Issueなし運用で継続する。
...
## 6. PR本文を生成する
1. `references/body-generation.md` に従い、差分・コミット履歴・テスト結果から本文を生成する。
