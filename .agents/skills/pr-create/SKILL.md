---
name: pr-create
description: PR作成依頼を受けたときに、検証・`$push`・PR作成までを一気通貫で実行する。Issueがある場合はIssueを優先して本文とbaseを決定し、Issueがない即興タスクでは継続/起票/中断を都度確認して進める。既存Open PRがある場合は更新か新規作成を確認する。
---
...
5. Issueが解決できない場合は、ユーザーへ次の3択を確認する。
   - Issueなしで継続
   - Issueを起票して継続
   - 中断
