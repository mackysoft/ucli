...
uCLIはすでに、[uCLI.md](uCLI.md) にある `plan -> call`、`planToken` による drift 検知、`projectFingerprint`、物理 `UnityProjectRoot` 単位の project lifecycle lock、`requestId` 冪等、`call` の live Unity 再解決、`dangerous` の隔離といった骨格を持っています。ここで先に凍結すべき制御面は、状態識別と project lifecycle lock identity の分離、transport 差を public 契約へ持ち込まないこと、`timeout != 未適用` を前提にした失敗時契約、idle 時の inert 性、reload-safe な session / log / artifact 管理です。これらが曖昧なまま機能を増やすと、同じ種別の不具合が transport や runtime を変えて再発しやすくなります。[^unity-cli-manual][^qiita-unity-mcp][^ivan500]
...
2. **物理 `UnityProjectRoot` を project lifecycle lock の第一級 identity にする（対応: D7）**
   同一物理projectに対する project lifecycle lock を port や起動順の偶然に委ねると、port conflict や二重起動は症状として何度でも再発します。`daemon / oneshot / test.run` を同じ lifecycle lock に参加させることが安全側の基準です。[^unity-cli-manual]
...
| D7 | 同一物理 project の project lifecycle lock が弱く、port conflict や二重起動を個別回避に委ねる | P0 | 同時実行で壊れる、二重起動 | Unity Manual[^unity-cli-manual] |
...
| project 同一性 | [uCLI.md](uCLI.md) に `projectPath` 解決順序と `projectFingerprint` による状態識別がある | 制御面の未明文化は今回の正本化で解消 |
| Unity project lifecycle lock | [uCLI.md](uCLI.md) に物理 `UnityProjectRoot` 単位の project lifecycle lock がある | 制御面の未明文化は今回の正本化で解消 |
...
| D4 | 正本化対象外（調査メモのみ） |
| D5 | 正本化対象外（調査メモのみ） |
| D6 | [uCLI.md](uCLI.md) `projectPath 解決順序` / `Project識別とreadIndex` / `planToken とドリフト検知` |
| D7 | [uCLI.md](uCLI.md) `物理 UnityProjectRoot 単位の project lifecycle lock` |
