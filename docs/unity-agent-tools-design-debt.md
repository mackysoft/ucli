...
1. **`compile / domain reload` をプロトコル状態として扱う（対応: D1, D2, D3）**
   `sleep` や ad-hoc polling に流れる時点で API ではなく運用に待機責務が漏れています。`ready / compiling / reloading / blocked` を first-class にし、将来の `waitUntilReady`、`compileGeneration`、`busy`、`playmode` のような状態拡張も、その延長として扱うべきです。[^coplay814][^unity-cli-manual]
...
|---|---|---|
| compile / reload の drift 制御 | [uCLI.md](uCLI.md) に `planToken`、`compileState`、`domainReloadGeneration` がある | `waitUntilReady`、`compileGeneration`、`busy / playmode` のような追加状態面は未固定 |
| project 同一性 | [uCLI.md](uCLI.md) に `projectFingerprint` と storage / daemon 同一性がある | 同一 `projectFingerprint` の `daemon / oneshot / refresh / test.run / mutation call` を同じ writer lane に入れる規範文が弱い |
| live Unity only | [uCLI.md](uCLI.md) に「`call` は readIndex に依存せず、Unity実体で再解決・再検証して実行する」とある | mutation 後に「次の read を何で安全とみなすか」という postcondition 契約は未整理 |
...
| 証拠と失敗時契約 | JSONレスポンス、`opResults`、test の `artifactsDir` など、構造化結果の骨格はある | `timeout != 未適用`、`indeterminate`、`unknownState` 相当の概念と artifact / log 証拠返却の一般契約は未固定 |
