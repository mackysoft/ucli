...
1. **Editor準備完了ゲート**  
   `call` は各opの `Call` 実行前に、`lifecycleState = ready` を満たさない場合、既定では `starting / busy / compiling / domainReloading` の解消を待つ。`--failFast` 指定時だけ `EDITOR_COMPILING` などの lifecycle 専用エラーで **1件も適用しない**。timeout は `IPC_TIMEOUT` とする。根拠として、コンパイル/ドメインリロード待ちがsleep依存になる再発があるため。citeturn6view2turn3view0
...
- **Editor状態ゲート**: コンパイル/ドメインリロード中に`call`が既定では待機し、`--failFast` 指定時だけ“1件も適用せず”に `EDITOR_COMPILING` / `EDITOR_DOMAIN_RELOADING` を返す。待機 timeout は `IPC_TIMEOUT` を返す（sleep依存の再発を防止）。citeturn6view2turn3view0
