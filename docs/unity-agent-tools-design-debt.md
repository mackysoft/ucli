...
1. **`compile / domain reload` をプロトコル状態として扱う（対応: D1, D2, D3）**
   `sleep` や ad-hoc polling に流れる時点で API ではなく運用に待機責務が漏れています。`ready / compiling / reloading / blocked` を first-class にし、`waitUntilReady`、`compileGeneration`、`busy`、`playmode` のような状態拡張も、その延長として扱うべきです。[^coplay814][^unity-cli-manual]

2. **`projectFingerprint` を第一級にし、同一projectは single-writer にする（対応: D6, D7）**
   同一projectに対する排他を port や起動順の偶然に委ねると、port conflict や二重適用は症状として何度でも再発します。`daemon / oneshot / refresh / test.run / mutation call` を同じ writer lane に参加させることが安全側の基準です。[^unity-cli-manual]
...
| D7 | 同一 project の single-writer 排他が弱く、port conflict や二重 writer を個別回避に委ねる | P0 | 同時実行で壊れる、二重適用 | Unity Manual[^unity-cli-manual] |
...
|---|---|---|
| compile / reload の drift 制御 | [uCLI.md](uCLI.md) に `planToken`、`compileState`、`domainReloadGeneration`、`waitUntilReady`、`compileGeneration`、`busy / playmode` がある | 制御面の未明文化は今回の正本化で解消 |
| project 同一性 | [uCLI.md](uCLI.md) に `projectFingerprint` と writer lane 契約がある | 制御面の未明文化は今回の正本化で解消 |
...
| 負債ID | 主な正本セクション |
|---|---|
| D1 | [uCLI.md](uCLI.md) `Editor Lifecycle` / [uCLI-command-reference.md](uCLI-command-reference.md) `--waitUntilReady` |
...
| D4 | 正本化対象外（調査メモのみ） |
| D5 | 正本化対象外（調査メモのみ） |
| D6 | [uCLI.md](uCLI.md) `projectFingerprint 単位の writer 排他` / `planToken とドリフト検知` |
| D7 | [uCLI.md](uCLI.md) `projectFingerprint 単位の writer 排他` |
...
1. **Editor準備完了ゲート**  
   `call` は各opの `Call` 実行前に、`lifecycleState = ready` を満たさない場合、`EDITOR_COMPILING` などの lifecycle 専用エラーで **1件も適用しない**。`--waitUntilReady` 指定時だけ `starting / busy / compiling / domainReloading` を待機対象にし、timeout は `IPC_TIMEOUT` とする。根拠として、コンパイル/ドメインリロード待ちがsleep依存になる再発があるため。citeturn6view2turn3view0
...
- **Editor状態ゲート**: コンパイル/ドメインリロード中に`call`が“1件も適用せず”に `EDITOR_COMPILING` / `EDITOR_DOMAIN_RELOADING` を返し、`--waitUntilReady` 指定時の timeout は `IPC_TIMEOUT` を返す（sleep依存の再発を防止）。citeturn6view2turn3view0
