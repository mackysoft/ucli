...
さらに uCLI は、AI / エージェントが行った Unity 変更を、そのまま差分やログとして人間へ渡すのではなく、`claim`、`evidence`、`coverage`、`residualRisk` へ変換する **assurance compiler** として扱う。ここでの出力は要約ではなく、保証判断に使うための機械可読な証拠契約である。
## 最上位原則
...
### Claim-first
uCLI の保証単位は command の成功メッセージではなく、scope を持つ `claim` である。
`claim` は、対象、主張、根拠、coverage、verifier、残余リスクを持ち、人間や上位 supervisor が「どこを承認すべきか」を判断するための最小単位になる。
### Cross-tool composability
uCLI は Unity Editor の live state、Scene / Prefab / Asset / Project mutation、compile / domain reload、Unity Test Runner、logs、post-read verification を扱う Unity-local assurance compiler である。
...
uCLI は claim-first の assurance packet を返す。
静的解析ツールは finding-first の review report を返してよい。
外部 supervisor はそれぞれの正本契約を保ったまま結果を束ねるべきであり、uCLI は他ツールの finding や verdict を再解釈しない。
### Lowerable
...
`planMode=mayCreatePreviewState` は Plan が review gate 前に状態を作るため、最低 `advanced` に導出する。public raw catalog では `mayCreatePreviewState` を禁止し、preview state が必要な primitive は `editLoweringOnly` に置く。
...
### Unknown / partial / indeterminate は第一級結果
保証できない状態を成功や失敗へ丸めない。
`unknown`、`partial`、`indeterminate`、`outOfScope` は、それぞれ独立した結果として扱う。

- `unknown`: code や verifier の意味を現在の client が知らない
- `partial`: 検証は行ったが coverage が不足している
- `indeterminate`: 実行、通信、reload、timeout などにより適用状態や検証状態が確定しない
- `outOfScope`: 意図的に保証対象外である

これらは failure の言い換えではない。人間のレビュー対象を、失敗した点だけでなく保証不足の点にも限定するための契約である。
### Passing tests alone are not assurance
テスト成功は、それがどの `claim` を支えるかが明示されて初めて保証材料になる。
`test.run`、`compile`、`ready`、post-read verification は、それぞれ evidence source であり、`verify` はそれらを claim packet に束ねる。
### 機械可読を優先する
