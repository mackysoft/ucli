...
uCLI の project 分割は、CLI hosting、application use case、共有契約、技術実装、Unity 実装、SKILL 配布の CLI 接続を混在させないために定義する。
...
| `MackySoft.AgentSkills` | 外部 NuGet package | SKILL 生成、検証、manifest、digest、host adapter、install / export / doctor の共通ロジック | uCLI 固有 command、IPC / operation contract の再定義、Unity 実装、CLI hosting、`Ucli.Contracts` / `Ucli.Infrastructure` への依存 |
...
| `MackySoft.AgentSkills` | 各 product の source definition を読み、canonical output を生成する。manifest、digest、host artifact を検証する。host ごとの materialization、install、export、doctor を実行する。operation contract は複製せず、agent workflow だけを配布可能にする。 |
...
`MackySoft.AgentSkills` は operation catalog を同梱せず、実行時に `ucli ops describe` を読む workflow layer を生成する。そのため他の uCLI production project へ依存しない。`ucli skills` CLI entrypoint は `Ucli` に置き、`Ucli` から `MackySoft.AgentSkills` version `0.1.0` を `PackageReference` で参照する。
...
SKILL template、manifest、digest、host adapter、materialization、install / export / doctor の共通ロジックは `MackySoft.AgentSkills` NuGet package に置く。uCLI 側には `skills/definitions`、`skills/generated`、`ucli skills` の CLI entrypoint、option 正規化、`CommandResult` 投影だけを置く。SKILL は operation contract の正本ではなく、operation args、result、assurance は実行時の `ucli ops describe` を正本にする。
...
`MackySoft.AgentSkills` は `MackySoft.Ucli` の外部 NuGet 依存として扱い、`PackageReference` version `0.1.0` で参照する。SKILL 配布物は `MackySoft.Ucli` package、release artifact、または `ucli skills install/export` の出力として扱う。
...
- `Ucli` が CLI host と composition root として `Ucli.Application`、`Ucli.Contracts`、`Ucli.Infrastructure`、`MackySoft.AgentSkills` 以外の production project へ依存しないこと
- `Ucli.Contracts` が `Ucli`、`Ucli.Application`、`Ucli.Infrastructure`、`Ucli.Unity`、`MackySoft.AgentSkills` に依存しないこと
...
- `MackySoft.AgentSkills` の内部構造は uCLI architecture tests の検査対象に含めず、agent-skills repository で検査すること
