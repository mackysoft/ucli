...
`ucli skills install` と `ucli skills export` は、公式 SKILL を常に一括で扱う。SKILL 種別ごとの導入分けは 1.0 の仕様に含めない。公式 SKILL として含めるには、全 supported host に対して安全に一括導入できることを条件にする。
...
| `ucli-verify-changes` | `test`、`logs`、再 query で変更後の証跡を確認する |
