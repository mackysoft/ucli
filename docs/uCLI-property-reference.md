...
`verify --from <ucli-result.json>` の `postRead` verifier は、`readPostcondition`、`opResults[]`、`postReadSource.steps[]` を読んで claim を生成する。ローカル保存済み request は参照しない。`--from` の `payload.project.projectFingerprint` と現在解決した project fingerprint が一致しない場合は `PROJECT_FINGERPRINT_MISMATCH` の command failure とし、postRead claim を生成しない。
...
`PERSISTENCE_UNIT_TOUCHED` は、`changed=true` かつ `postReadSource.steps[].persistenceExpected=true` の mutation では `required=true` とする。Play Mode live object 変更のように永続化単位を期待しない mutation では `outOfScope` または `required=false` とする。`READ_SURFACE_SAFE` は `readPostcondition.requirements[]` が存在する場合に `required=true` とする。`POST_MUTATION_OBSERVED` は `postReadSource.steps[].expectedPostState=deterministic` の applied/changed mutation だけを `required=true` 候補にする。raw op や broad mutation など期待 post-state が定義されない場合は `outOfScope` または `required=false` とし、実装ごとの推測で required claim にしてはならない。

## 内部 IPC 契約

### `IpcRequest`
CLI と Unity runtime の間の外側 IPC request envelope は、method-specific payload とは別に response framing を指定する。

| Property | Type | Required | Description |
| --- | --- | --- | --- |
| `protocolVersion` | integer | yes | IPC プロトコルメジャーバージョン |
| `requestId` | string | yes | IPC リクエスト相関 ID |
| `sessionToken` | string | yes | daemon / oneshot IPC セッション認証トークン |
| `method` | string | yes | IPC method name |
| `payload` | object | yes | IPC method-specific request payload |
| `responseMode` | `single \| stream` | yes | response framing mode。`stream` は progress frame のあと terminal frame で最終 `IpcResponse` を返す |

stream 要否は method payload に含めず、外側 IPC request envelope の `responseMode` で表す。
...
| `postReadSource` | object | no | `verify --from` が postRead claim を生成するための source facts。shape は `IpcExecuteResponse.postReadSource` を参照 |
...
`mayPersist=false` との矛盾は、operation 名や `touched[].kind` から推定せず、内部 execution trace が保存・書き込み成功を観測した場合だけ `observedResult="executionTrace.persisted=true"` として返す。現在の runtime validator は、`persisted=true` または `applied=true` を `applicationState=applied`、`changed=true` を `applicationState=indeterminate`、それ以外を `applicationState=notApplied` として返す。
...
#### `IpcExecuteResponse.postReadSource`

`postReadSource` は portable な mutation result JSON だけから `verify --from` を成立させるための最小 source facts である。`verify --from` はローカル保存済み request を探さず、この field と `opResults[]` の照合結果だけを使う。

| Property | Type | Required | Description |
| --- | --- | --- | --- |
| `schemaVersion` | `1` | yes | source facts schema version |
| `steps` | array | yes | `opResults[].opId` と対応する source facts |

#### `IpcExecuteResponse.postReadSource.steps[]`

| Property | Type | Required | Description |
| --- | --- | --- | --- |
| `opId` | string | yes | 対応する `opResults[].opId` |
| `sourceKind` | `edit \| operation \| refresh` | yes | source 種別。raw operation と broad refresh は期待 post-state を導けない |
| `playModeMutation` | boolean | yes | Play Mode live state の mutation か |
| `commit` | `none \| context \| project \| null` | yes | edit commit 種別。edit 以外では `null` |
| `persistenceExpected` | boolean | yes | `changed=true` のとき永続化単位の touch を期待するか |
| `expectedPostState` | `deterministic \| unavailable` | yes | `POST_MUTATION_OBSERVED` を required 候補にできる deterministic post-state があるか |
...
| `readPostcondition` | object | no | mutation 後 read の safe 条件。shape は `IpcExecuteResponse.readPostcondition` を参照 |
| `postReadSource` | object | no | `verify --from` 用 source facts。shape は `IpcExecuteResponse.postReadSource` を参照 |
...
public v1 catalog の `planMode` は次の値だけを使う。
...
| `runId` | string \| null | yes | 実行 ID。`runId` 確定前に失敗した場合は `null` |
| `artifactsDir` | string \| null | yes | 成果物ディレクトリ。artifact session 確定前に失敗した場合は `null` |
| `summaryJsonPath` | string \| null | yes | サマリー JSON のパス。artifact session 確定前に失敗した場合は `null` |
...
| --- | --- | --- | --- |
| `runId` | string | yes | 実行 ID |
| `code` | string | yes | stream-local 診断 code。event 名とは異なるが、v1 では `ucli codes` catalog の対象ではない |
| `message` | string | yes | 人間向け説明 |
| `severity` | `info \| warning \| error` | yes | 診断 severity |
