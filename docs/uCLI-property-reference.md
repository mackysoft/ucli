...
| `warnings` | array | no | 非致命的な保証低下。未発生時は field 自体を省略する |

#### `IpcExecuteResponse.opResults[].warnings[]`

| Property | Type | Required | Description |
| --- | --- | --- | --- |
| `code` | string | yes | warning 識別子 |
| `message` | string | yes | 人間向け説明 |

GUI session の `query` / `resolve` / `plan` が観測由来の Editor state を復元できない場合は、該当 `opResults[].warnings[]` に warning を返す。
