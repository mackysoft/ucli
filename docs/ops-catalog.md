...
- `declaredKind = query` かつ `mayDirty = true`
- `declaredKind = query` かつ `mayPersist = true`
...
- `mayPersist = true` かつ `touchedKinds` が空
- `exposure = public` かつ `planMode = mayCreatePreviewState`
...
v1 public raw operation の `planMode` は `validationOnly` と `observesLiveUnity` だけである。`mayCreatePreviewState` は Plan が review gate 前に一時 Scene / Prefab Stage などの状態を作るため、v1 public raw catalog には出さない。将来の experimental / internal operation で使う場合でも、`policy` は最低 `advanced` とし、cleanup evidence、`failureSemantics` の residual risk を必須にする。cleanup と適用境界を保証できない場合は `dangerous` とする。

`sideEffects` から policy を導出する最小基準は次の表を正本とする。複数の side effect がある場合は、最も厳しい minimum policy を採用する。
...
新しい `sideEffects` value を追加する場合は、同じ変更で minimum policy、notes、contract test fixture を追加しなければならない。side effect value だけを追加し、policy 導出が未定義になる catalog は validation failure とする。
...
- `mayDirty=false`、`mayPersist=false`、advanced / dangerous side effect なしは `safe`
- `assetDatabaseRefresh`、`assetImport`、`scriptCompilation`、`domainReload` は `advanced`
- `mayDirty=true` または `mayPersist=true` かつ bounded `touchedKinds` がある Unity Editor API write は `advanced`
...
- 新しい `sideEffects` value は minimum policy、notes、contract test fixture を同時に持つ
