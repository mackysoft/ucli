...
`query` は観測だけを行い、完了後に Editor 状態や永続化単位への変更を残さない。`command` は Scene のロード、Prefab 編集コンテキスト、AssetDatabase refresh / import のような一時的な Editor / runtime 状態を動かすが、ユーザーデータの永続化単位を直接編集・保存する操作ではない。`mutation` は Unity object、asset、project-scoped asset、または永続化単位を変更する。
