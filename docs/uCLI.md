...
- `running` には、ping 成功に加えて、daemon が応答したが persisted sessionToken が一致しないケースを含む
- `uncertainReachability` には、reachability probe の timeout に加えて、`session.json` 不在により live daemon の非稼働を証明できないケースを含む
