...
| `playmode` | Play Mode 中で通常実行を許可しない | Editor の Play 実行開始 | `validate`, `status`, `daemon status`, `logs`, `daemon stop`, `play status`, `play enter`, `play exit`, `--allowPlayMode` 付きの Play Mode 変更 `plan` / `call` | `EDITOR_PLAYMODE` | Play Mode 終了後に `starting` または `ready` / 他の優先状態へ移る |
...
`ucli play` と `--allowPlayMode` の責務は分ける。`--allowPlayMode` は Play Mode 中 mutation request を明示許可する `plan` / `call` 用ガードであり、Play Mode lifecycle 制御を許可する option ではない。`play enter` / `play exit` は command 自体が明示的な lifecycle 制御であるため、`--allowPlayMode` を受け取らない。
