...
- `call` の実行順序は次で固定する
  ...
  - `ready` の場合に全opに対して `Validate` / `Plan` を実行（fail-fast）
  - `planToken` を検証
...
### 状態項目
- `runtime`：`batchmode | gui`
- `compileState`：`ready | compiling`
  - `compileState` は compiler activity 専用の項目であり、`lifecycleState` の代替ではない
- `domainReloadGeneration`：domain reload 完了ごとに変化する opaque な識別子
  - 呼び出し側は値を解釈せず、変化有無のみを扱う
- `blockingReason`：`null | startup | compile | domainReload | modalDialog | safeMode | shutdown`
  - `lifecycleState` が `ready` の場合は `null`
  - `daemonStatus != running` の場合は `null`
- `canAcceptExecutionRequests`
  - `daemonStatus = running` かつ `lifecycleState = ready` のときのみ `true`
  - それ以外は `false`
...
- `ucli status`：
  ...
  - JSON payload には `daemonStatus`, `unityVersion`, `serverVersion`, `runtime`, `lifecycleState`, `blockingReason`, `compileState`, `domainReloadGeneration`, `canAcceptExecutionRequests`, `timeoutMilliseconds` を返す
  - `unityVersion` は常に `ProjectSettings/ProjectVersion.txt` 由来とする
  - `serverVersion` / `runtime` / `lifecycleState` / `blockingReason` / `compileState` / `domainReloadGeneration` / `canAcceptExecutionRequests` は `daemonStatus = running` のときのみ値が入り、`notRunning | stale` では `null`
...
#### `daemon cleanup` の `payload`
- `cleanupStatus`：`completed | skipped`
- `skipReason`：`null | running | unsafeInvalidSession | uncertainReachability`
- `timeoutMilliseconds`：実効タイムアウト（ミリ秒）
- `completed` は安全に実行可能な cleanup パスを完了したことを表す
- `skipped` は成功扱いとし、cleanup を見送った理由を `skipReason` に返す
- `session.json` 不在や JSON 破損で session metadata を使えない場合でも、cleanup 専用 probe が daemon 不在を証明できたときは cleanup を実行する
- `uncertainReachability` には、reachability probe の timeout に加えて、session metadata 不在時や persisted sessionToken 不一致時、named pipe connect timeout など daemon 不在を safe cleanup として証明できない既知の曖昧ケースを含む
- cleanup は `session.json` と daemon endpoint residue のみを対象とし、`daemon-diagnosis.json` と supervisor の repository ローカル metadata は削除しない

#### `daemon status` の `payload`
- `daemonStatus`：`running | notRunning | stale`
- `serverVersion`：`daemonStatus = running` のときのみ値が入り、`notRunning | stale` では `null`
- `runtime`：`batchmode | gui`
  - `daemonStatus = running` のときのみ値が入り、`notRunning | stale` では `null`
- `lifecycleState`：`starting | ready | compiling | domainReloading | blockedByModal | safeMode | shuttingDown | null`
  - `daemonStatus = running` のときのみ値が入り、`notRunning | stale` では `null`
- `blockingReason`：`null | startup | compile | domainReload | modalDialog | safeMode | shutdown`
  - `daemonStatus = running` のときのみ意味を持つ
- `compileState`：`ready | compiling | null`
- `domainReloadGeneration`：opaque な識別子。`daemonStatus = running` のときのみ値が入り、`notRunning | stale` では `null`
- `canAcceptExecutionRequests`：`daemonStatus = running` かつ `lifecycleState = ready` のときのみ `true`
- `timeoutMilliseconds`：実効タイムアウト（ミリ秒）
- `session`：
...
生成対象は `.ucli/config.json`, `.ucli/.gitignore`。
