...
- 公開 CLI 契約コマンドは、最後に `stdout` へ共通 `CommandResult` JSON を1件だけ出力する
- 一部のコマンドは、最終 `CommandResult` の前に途中出力として entry stream を持つ
- entry stream はログエントリ、テスト進捗、長時間処理の観測イベントを表し、最終結果の代替にはしない
...
- entry stream を出力する invocation では、呼び出し側は `stderr` を並行して読み出さなければならない。`stdout` だけを待機する runner は `stderr` pipe の詰まりで停止し得る
- entry stream は進捗表示と早期観測のための progress view であり、最終判定や assurance evidence の正本ではない。正本は最終 `CommandResult.payload` と保存された artifacts とする
...
### CLI出力契約
- 最終 `CommandResult` の `status` は `ok | error` を使用する
  - `ok`：コマンドが契約どおり完了した
  - `error`：入力不正、インフラ障害、外部ツール障害などで完了できなかった
- 最終 `CommandResult` の終了コードはコマンド別契約に従う
...
- entry stream を出力するコマンドも、終了時は必ず最終 `CommandResult` を返す
- entry stream 開始後に失敗した場合も、最終 `CommandResult` は `status=error` として失敗内容を説明する
...
`errors[]` には長い説明や全候補原因を埋め込まない。実行時レスポンスは発生固有の `code`、`message`、`opId`、および可能な場合の `payload.opResults`、`payload.readPostcondition`、診断情報だけを返す。静的な意味は `ucli codes describe <CODE>` で取得する。operation 実行結果が `ops describe` の `assurance` facts に反する場合は `OPERATION_CONTRACT_VIOLATION` を返し、`payload.contractViolations[]` に矛盾した expected fact、observed result、`applicationState` を返す。この error は未適用を意味せず、`applicationState=notApplied` 以外は再試行安全性を保証しない。

### 公開 CLI 共通エンベロープ
最終 `CommandResult` の共通エンベロープと JSON entry の共通エンベロープのフィールド定義は [uCLI-property-reference.md](uCLI-property-reference.md) を参照する。
...
request command payload schema は、`payload.project`、`payload.requestId`、`payload.opResults[]`、`payload.readIndex`、`payload.planToken`、`payload.readPostcondition`、`payload.contractViolations[]` などの command result envelope を検証する。`opResults[].result` の中身は operation ごとの `resultSchema` へ委譲し、command payload schema に複製しない。
...
外側 IPC request envelope は `responseMode=single|stream` を持つ。`single` は従来どおり単一 `IpcResponse` を返し、`stream` は progress `IpcStreamFrame` を0件以上返したあと terminal `IpcStreamFrame` に最終 `IpcResponse` を載せる。stream 要否は method payload ではなく IPC envelope で表す。
...
- `contractViolations` は execute 応答に属し、`OPERATION_CONTRACT_VIOLATION` の場合だけ公開するコマンドの `payload.contractViolations[]` に写像する
...
| `playmode` | Play Mode 中で通常実行を許可しない | Editor の Play 実行開始 | `validate`, `status`, `daemon status`, `logs`, `daemon stop`, `play status`, `play enter`, `play exit`, `--allowPlayMode` 付きの Play Mode 変更 `plan` / `call` / `eval` | `EDITOR_PLAYMODE` | Play Mode 終了後に `starting` または `ready` / 他の優先状態へ移る |
...
- `--allowPlayMode` が指定され、Play Mode 変更契約を満たす `plan` / `call` / `eval` だけは `playmode` を即時失敗状態として扱わない
...
- `test.run` は `daemon` / `oneshot` のどちらでも IPC `test.run` として実行し、`--failFast` は解決された target の readiness gate に適用する

### 実行系コマンドの状態別挙動
- `status`, `daemon status`, `logs`, `daemon stop` は常時利用可能とする
- 未公開の request static validation は Editor lifecycle gate の対象外とする
- 内部 `plan`, `call`, `resolve`, `query` と公開 `refresh`、`test.run` は `lifecycleState = ready` 必須とする
- `--allowPlayMode` 付き `plan` / `call` / `eval` は例外として `editorMode = gui` かつ `lifecycleState = playmode` の Play Mode 変更だけを許可する
- `play status` は GUI Editor session で Play Mode snapshot の観測だけを行い、通常実行 request の readiness gate には入らない
- `play enter` / `play exit` は通常実行 request の readiness gate ではなく Play Mode lifecycle 専用 gate を使う
...
- `test.run` の `oneshot` target も batchmode bootstrap 後に IPC readiness probe を行い、`--failFast` 指定時は待機可能状態を即時失敗へ切り替える
...
### Play Mode 制御
`ucli play` は Unity Editor Play Mode の明示 lifecycle control command family である。Play Mode enter / exit は uCLI content mutation を実行せず、`opResults[].touched` を返す content edit として扱わない。Play Mode に入ると project code が実行され、AssetDatabase、filesystem、static state、Editor state などに副作用を起こす可能性はあるが、その副作用は uCLI request-step mutation attribution の対象外とする。

Play Mode enter / exit は Editor lifecycle state を遷移させる操作であるため、JSON request step や primitive `op` として表現しない。
...
`play status` は状態観測だけを行い、Play Mode enter / exit を行わない。`play enter` は GUI Editor session を Play Mode へ入れ、`EditorApplication.isPlaying == true` まで bounded wait する。`play exit` は Play Mode を終了し、exit 後に compile / domain reload / startup / busy を経て `lifecycleState = ready` に戻るまで bounded wait する。
...
`ucli play` は GUI Editor session 専用である。batchmode session で `play status` / `play enter` / `play exit` を呼び出した場合は `PLAYMODE_REQUIRES_GUI_EDITOR` を返す。batchmode session の lifecycle 観測は `ucli status` または `ucli daemon status` で行う。

`ready` は readiness gate であり、Play Mode を暗黙に終了しない。`verify` は検証であり、Play Mode を暗黙に enter / exit しない。`test run` は Unity Test Framework 実行の command であり、PlayMode test 実行のために `ucli play enter` を内部利用しない。Play Mode blocker を解消したい呼び出し側は `ucli play exit` を明示的に実行する。

`ucli play` と `--allowPlayMode` の責務は分ける。`--allowPlayMode` は Play Mode 中 mutation request を明示許可する `plan` / `call` / `eval` 用ガードであり、Play Mode lifecycle 制御を許可する option ではない。`play enter` / `play exit` は command 自体が明示的な lifecycle 制御であるため、`--allowPlayMode` を受け取らない。

Play Mode transition timeout は transport timeout と区別する。IPC request / response 自体が成立しない場合は `IPC_TIMEOUT`、Unity は応答しているが desired Play Mode state に到達しない場合は `PLAYMODE_TRANSITION_TIMEOUT` を返す。transition timeout は no-op を意味しない。payload には latest observed lifecycle snapshot と retry 判断に必要な transition state を返す。

### Play Mode 変更
Play Mode 中の変更 request は通常の `plan` / `call` では拒否し、`--allowPlayMode` を指定した場合だけ Play Mode 変更として扱う。`ucli.cs.eval` は dangerous guard と `--allowPlayMode` の両方を通過した場合だけ、Play Mode 変更における raw `kind:"op"` 例外として実行できる。`--allowPlayMode` は mutation request 用の明示ガードであり、`query` / `resolve` / `validate` / `ops` には適用しない。

- 実行条件
  - `editorMode = gui`
  - `lifecycleState = playmode`
  - command は `plan` / `call`、または `ucli.cs.eval` を実行する `eval`
...
- `--allowDangerous` は dangerous policy の gate だけを開く。`editLoweringOnly` の exposure gate は解除しない
...
- `kind`：validation 済みの operation kind。`query | command | mutation`
...
- `policy`：operation contract facts から導出された admission policy。`safe | advanced | dangerous`
...
`argsSchema` と `resultSchema` は検証用の派生成果物であり、agent 向けの主契約ではない。agent は `description` / `inputs[].constraints` / `inputs[].variants[].fields[].constraints` / `resultContract` / `assurance` を使って operation を選び、`args` を組み立て、admission を判断する。source code を受け取る operation では `codeContract` を source form と C# API 契約として読む。`argsSchema` は組み立て後の JSON 構造検証に使う。schema には説明文や意味制約を置かない。
...
`mayCreatePreviewState` は Plan が review gate 前に preview state を作るため、`policy` を最低 `advanced` に導出する。public raw catalog では `mayCreatePreviewState` を禁止し、preview state が必要な primitive は `editLoweringOnly` に置く。
...
- `ucli ready`：次の操作へ進める Unity readiness gate を claim として返す
  ...
  - `payload.verdict`、`project`、`verifiers[]`、`claims[]`、`reports`、`residualRisks[]` に加えて、`target`、`requestedMode`、`resolvedMode`、`sessionKind`、`timeoutMilliseconds`、`lifecycle`、`readIndex` を返す
  - `verifiers[]` は runtime lifecycle を観測した場合は `id=ready.lifecycle`、readIndex artifact を観測した場合は `id=ready.readIndex` を返す
...
- `ucli verify`：Unity 側 verifier の結果を claim packet に束ねる
  ...
  - `postRead` は `readPostcondition`、`opResults[]`、`postReadSource.steps[]` から claim を生成し、ローカル保存済み request には依存しない
- `ucli ops`：利用可能なオペレーション一覧・詳細
  ...
  - `describe` は `kind` / `policy`、`inputs`、`resultContract`、`assurance`、`codeContract`、`argsSchema`、`resultSchema` を返す
...
`ops.catalog.json` は `ops list` と `ops describe` の detail 参照に使う軽量目録であり、public raw `kind:"op"` として呼べる operation ごとの `name` / `kind` / `policy` / `description` と describe detail の参照情報を持つ。`ops.describe/<opKey>.json` は public `ops describe` 用の単一 operation detail artifact であり、`description`、`inputs`、`resultContract`、`assurance`、`codeContract`、`argsSchema`、`resultSchema` を保持する。`opKey` は operation name から決定論的に作られる不透明な safe key であり、利用者は path を直接組み立てず `ucli ops describe <opName>` を読む。
