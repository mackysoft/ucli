...
- `uncertainReachability` には、reachability probe の timeout に加えて、session metadata 不在時や persisted sessionToken 不一致時、recorded daemon process の停止を証明できない named pipe connect timeout など daemon 不在を safe cleanup として証明できない既知の曖昧ケースを含む
