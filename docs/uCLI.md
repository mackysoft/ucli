...
- `starting` / `busy` / `compiling` / `domainReloading` は既定で待機対象とし、`--failFast` 指定時だけライフサイクル専用エラーで失敗させる
...
- `starting`, `busy`, `compiling`, `domainReloading` は既定で待機対象とし、`--failFast` 指定時だけライフサイクル専用エラーで拒否する
