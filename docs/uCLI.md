...
- Unity の `Temp/UnityLockfile` は Unity 所有の二重起動防止 marker として扱う。uCLI は Unity process を新規起動する直前にだけ preflight を行い、stale と断定できる場合だけ `Temp/UnityLockfile` を自動削除して起動を続行する
