...
- `ucli daemon`：常駐サーバ管理
  - `start`：対象 `projectFingerprint` のデーモンを起動
    ...
    - 新規 Unity process 起動時の Editor executable は、`ProjectSettings/ProjectVersion.txt` の `m_EditorVersion` と既定の Unity install search roots から自動解決する。v1.1 の `daemon start` は `--unityEditorPath` を公開引数として持たない
  - `stop`：デーモンを停止
