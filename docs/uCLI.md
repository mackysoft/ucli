...
- 現在の公開 CLI host が登録している command は `init`、`status`、`refresh`、`validate`、`plan`、`call`、`daemon`、`logs`、`ops`、`test` である
- JSON リクエストと `resolve` / `query` の記述は、内部 execute 契約または未公開 surface の設計を含む
...
`resolve` / `query` は未公開の request 系 CLI 設計であり、現行 host には登録されていない。
