...
`policy` は author が任意に選ぶ安全ラベルではなく、operation contract facts から導出される入場制御レベルである。catalog builder は `assurance.sideEffects`、`mayDirty`、`mayPersist`、`touchedKinds`、`planMode`、`codeContract`、`exposure`、destructive scope、arbitrary execution、external process / filesystem access から `operation.policy` を導出する。author が `policy` を直接指定する API や、主観的に policy を厳しくする上書き指定は持たない。実行結果が assurance facts に反する場合は `OPERATION_CONTRACT_VIOLATION` とする。
