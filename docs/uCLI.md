...
### 物理 UnityProjectRoot 単位の Unity 起動排他
...
GUI Editor session も batchmode Editor session と同じ `projectFingerprint`、session token、同一ユーザー IPC 認可、物理 `UnityProjectRoot` 単位の起動排他に参加する。
...
`argsSchema` と `resultSchema` は検証用の派生成果物であり、agent 向けの主契約ではない。agent は `description` / `inputs[].constraints` / `inputs[].variants[].fields[].constraints` / `resultContract` / `assurance` を使って operation を選び、`args` を組み立てる。`argsSchema` は組み立て後の JSON 構造検証に使う。schema には説明文や意味制約を置かない。
