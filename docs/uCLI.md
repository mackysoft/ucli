...
### capability 交渉規則
- `protocolVersion` は wire 互換性だけを表す
- capability は transport / runtime / optional feature の利用可否を表す別軸とする
- transport 差で意味を変えず、差異がある場合は黙って欠落させず capability 差として明示する
- 具体的な public field 名は別途固定するが、本書では capability 差が silent fallback にならないことを先に固定する
...
- timeout / disconnect / crash は `indeterminate` を含みうる失敗として扱い、artifact / log / diagnosis の証拠返却を優先する
- `unknownState` などの具体的な public field 名は別途固定する
...
- `busy` / `playmode` / `waitUntilReady` / `compileGeneration` は次の仕様課題として扱う
  - 本版では public 状態語彙や public field 名として固定しない
