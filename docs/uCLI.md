...
Args/Result contract 型は `MackySoft.Ucli.Contracts` に置き、公開 JSON 構造の正本にする。operation の説明、入力の意味制約、result の読み方、副作用保証は operation metadata に置く。operation 実装や CLI query builder は JSON 境界に入る前に typed contract を扱う。`JsonElement` は IPC codec、schema validation、低レベル deserialize 境界に限定する。
