...
### execute 系応答の公開写像
`plan` / `call` / `resolve` / `query` / `refresh` は、内部では `IpcResponse.payload = IpcExecuteResponse` を受け取る。公開 CLI では、その内部応答を各コマンドの `payload` へ写像する。
...
- `planToken` は execute 応答に属し、公開 `ucli plan` では `payload.planToken` に写像する
...
- `starting` / `busy` / `compiling` / `domainReloading` は `--waitUntilReady` 指定時だけ待機対象とし、未指定時はライフサイクル専用エラーで失敗させる
...
### `waitUntilReady`
- `waitUntilReady` は CLI option のみであり、JSON request に追加しない
- `.ucli/config.json` に既定値は持たない
- 既定動作は fail-fast とし、`--waitUntilReady` を指定した場合だけ待機を有効化する
- 待機対象は `starting` / `busy` / `compiling` / `domainReloading` とする
...
- `plan`, `call`, `resolve`, `query`, `refresh`, `test.run` は `lifecycleState = ready` 必須とする
- 上記コマンドは `--waitUntilReady` を受け付ける。未指定時は `lifecycleState != ready` で即時失敗し、指定時のみ wait 対象状態を待機する
- `ops list` / `ops describe` は readIndex hit の場合は lifecycle gate を通さず返す
- `ops list` / `ops describe` が live source fallback を行う場合だけ `--waitUntilReady` 契約を適用する
...
- `starting`, `busy`, `compiling`, `domainReloading` は `--waitUntilReady` 指定時だけ待機対象とし、未指定時はライフサイクル専用エラーで拒否する
...
```json
{
  ...
  "status": "error",
  "exitCode": 4,
  "message": "Unity editor is compiling scripts. Retry with --waitUntilReady or wait until lifecycleState=ready before executing request.",
  "payload": {
...
`--waitUntilReady` 指定時に timeout budget を使い切ったレスポンス例:
