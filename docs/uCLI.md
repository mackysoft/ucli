...
- `argsSchema`：引数 `args` のJSONスキーマ。正本は operation ごとの Args contract 型であり、JSON Schema はその派生成果物とする
- `resultSchema`：`opResults[].result` のJSONスキーマ。結果を返さない operation は `null` とし、実行結果でも `result` field を省略する

Args/Result contract 型は `MackySoft.Ucli.Contracts` に置く。必須性、説明、JSON property 名、単純制約は contract 型の property 属性で表現し、operation 実装や CLI query builder は JSON 境界に入る前に typed contract を扱う。`JsonElement` は IPC codec、schema validation、低レベル deserialize 境界に限定する。
...
- `IUcliOperation` を実装する。新規 operation は `TypedUcliOperation<TArgs,TResult>` を優先し、phase 本体が typed Args を受け取る形にする
