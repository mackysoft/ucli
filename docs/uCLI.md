...
| `playmode` | Play Mode 中で通常実行を許可しない | Editor の Play 実行開始 | `validate`, `status`, `daemon status`, `logs`, `daemon stop`, `play status`, `play enter`, `play exit`, `play wait`, `--allowPlayMode` 付きの Play Mode 変更 `plan` / `call` | `EDITOR_PLAYMODE` | Play Mode 終了後に `starting` または `ready` / 他の優先状態へ移る |
...
- `play enter` / `play exit` / `play wait` は通常実行 request の readiness gate ではなく Play Mode lifecycle 専用 gate を使う
...
`play status` は状態観測だけを行い、Play Mode enter / exit を行わない。`play enter` は GUI Editor session を Play Mode へ入れ、`EditorApplication.isPlaying == true` まで bounded wait する。`play exit` は Play Mode を終了し、exit 後に compile / domain reload / startup / busy を経て `lifecycleState = ready` に戻るまで bounded wait する。`play wait` は指定状態の観測と bounded wait だけを行い、暗黙の enter / exit は行わない。
...
`ucli play` は GUI Editor session 専用である。batchmode session で `play status` / `play enter` / `play exit` / `play wait` を呼び出した場合は `PLAYMODE_REQUIRES_GUI_EDITOR` を返す。batchmode session の lifecycle 観測は `ucli status` または `ucli daemon status` で行う。
...
`ucli play` と `--allowPlayMode` の責務は分ける。`--allowPlayMode` は Play Mode 中 mutation request を明示許可する `plan` / `call` 用ガードであり、Play Mode lifecycle 制御を許可する option ではない。`play enter` / `play exit` / `play wait` は command 自体が明示的な lifecycle 制御であるため、`--allowPlayMode` を受け取らない。

`play wait --until entered` は `playMode.state = playing`、`playMode.transition = none`、`playMode.isPlaying = true` を待つ。`play wait --until exited` は `playMode.state = stopped`、`playMode.transition = none`、`playMode.isPlaying = false`、`playMode.isPlayingOrWillChangePlaymode = false` を待つ。`play wait --until ready` は `--until exited` の条件に加えて、`lifecycleState = ready`、`blockingReason = null`、`canAcceptExecutionRequests = true` を待つ。
