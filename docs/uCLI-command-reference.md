...
- `--allowPlayMode` 付きの `ucli plan` / `ucli call` は、GUI Editor session の `playmode` に限り Play Mode 変更として実行できる。この option は mutation request 用の明示ガードであり、`query` / `resolve` / `validate` / `ops` には適用しない。raw `kind:"op"` は原則として Play Mode 変更では許可しないが、dangerous guard を通過した `ucli.cs.eval` だけは明示例外として扱う。
...
- `ucli test run` は `daemon` / `oneshot` のどちらでも Unity IPC `test.run` として実行する。`--failFast` は IPC 実行前の readiness gate に渡し、`oneshot` の startup probe でも待機可能状態を即時失敗へ切り替える。
...
- `--format text` のログ entry は物理1行を維持する。ログ本文、stack trace、raw payload などに含まれる CR / LF / ANSI escape / OSC sequence / その他制御文字 / Unicode format・line separator・paragraph separator は、端末制御や表示方向制御として解釈されない形にエスケープまたは置換する。
...
| `--format <string?>` | - | entry 表示形式。`json` / `text`。未指定時は `text` |
...
- `--format=json` は `--stream` 有無にかかわらず、`stderr` のログ entry を NDJSON で出力する。
...
- Play Mode 変更では raw `kind:"op"` を原則許可せず、Prefab apply / revert primitive は `edit` lowering から発生した場合だけ許可する。`ucli.cs.eval` だけは明示例外として扱う。
...
- `--allowPlayMode` 指定時の raw `kind:"op"` 例外は `ucli.cs.eval` だけである。他の raw operation は Play Mode 変更では引き続き拒否する。
...
### `ucli test run`
Unity IPC `test.run` を実行し、Unity Test Framework の実行結果を正規化して返す。`daemon` は既存 Unity daemon へ IPC request を送り、`oneshot` は batchmode bootstrap で起動した Unity process へ同じ IPC request を送る。
...
| `--failFast` | - | readiness gate で `ready` 待機を行わず即失敗する |
| `--format <string?>` | - | テスト進捗 entry 表示形式。`json` / `text`。未指定時は `text` |
...
#### `run` の出力
- 最後に `stdout` へ共通 `CommandResult` を1件出力する。
- `payload` のフィールド定義は [uCLI-property-reference.md](uCLI-property-reference.md) を参照する。
- `runId` 確定後は、最終 `CommandResult` の前にテスト進捗 entry を `stderr` に出力する。
...
- `--format text` のテスト進捗 entry は物理1行を維持する。test name、message、stack trace などに含まれる CR / LF / ANSI escape / OSC sequence / その他制御文字 / Unicode format・line separator・paragraph separator は、端末制御や表示方向制御として解釈されない形にエスケープまたは置換する。
...
- 呼び出し側は `stderr` を並行して読み出さなければならない。`stdout` だけを待機する runner は `stderr` pipe の詰まりで停止し得る。
...
- `--failFast` は `daemon` の readiness wait と `oneshot` の startup probe に適用する。`auto` の場合は解決された実行 target の readiness gate に従う。

#### `run` の最終結果 matrix
| 状態 | `status` | `payload.result` | `payload.errorKind` | Exit code |
| --- | --- | --- | --- | --- |
| テスト成功 | `ok` | `pass` | `null` | `0` |
| テスト失敗 | `ok` | `fail` | `null` | `1` |
| 入力不正 | `error` | `null` | `invalidInput` | `3` |
| Unity / test infrastructure 不成立 | `error` | `null` | `infraError` | `2` |
| daemon / oneshot mode または tool 制約違反 | `error` | `null` | `toolError` | `4` |
